const socket = io();
let playerId = '', room = '', roles = {}, stats = {}, history = [];

const statusEl = document.getElementById('status');
const p1x = document.getElementById('p1x'), p1o = document.getElementById('p1o');
const p2x = document.getElementById('p2x'), p2o = document.getElementById('p2o');
...
// Bind cell clicks
board.querySelectorAll('.cell').forEach(c => c.onclick = () => {
  const idx = +c.dataset.index;
  socket.emit('move', { playerId, room, index: idx, symbol: roles[playerId] });
});
...
socket.on('start', d => {
  playerId = d.playerId; room = d.room; roles = d.roles; stats = d.stats; history = d.history;
